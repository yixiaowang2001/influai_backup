...
@app.get("/posts/{post_id}/likes",
         summary="获取帖子点赞信息",
         description="获取指定帖子的点赞数和当前用户的点赞状态。",
...
@app.post("/posts/likes-stats",
          summary="批量获取帖子点赞信息",
          description="批量获取多个帖子的点赞数和当前用户的点赞状态。用于异步加载点赞信息，避免阻塞主要的帖子列表查询。",
    ...
    try:
        ...
        for post_id in post_ids:
            try:
                ...
                logger.warning(f"跳过无效的帖子ID: {post_id}")
                continue
        
        # 批量查询点赞信息
        likes_data = crud.get_posts_likes_batch(db, numeric_ids)
        
        # 转换为前端期望的格式
        result = {}
        for numeric_id in numeric_ids:
            post_id_str = f"post_{numeric_id}"
            if numeric_id in likes_data:
                result[post_id_str] = likes_data[numeric_id]
            else:
                result[post_id_str] = {"likes": 0, "isLiked": False}
...
@app.get("/comments/{comment_id}/likes",
         summary="获取评论点赞信息",
         description="获取指定评论的点赞数和当前用户的点赞状态。",
...
@app.post("/comments/likes-stats",
          summary="批量获取评论点赞信息",
          description="批量获取多个评论的点赞数和当前用户的点赞状态。用于异步加载点赞信息，避免阻塞主要的评论列表查询。",
    ...
    try:
        ...
        for comment_id in comment_ids:
            try:
                ...
                logger.warning(f"跳过无效的评论ID: {comment_id}")
                continue
        
        # 批量查询点赞信息
        likes_data = crud.get_comments_likes_batch(db, numeric_ids)
        
        # 转换为前端期望的格式
        result = {}
        for numeric_id in numeric_ids:
            comment_id_str = f"comment_{numeric_id}"
            if numeric_id in likes_data:
                result[comment_id_str] = likes_data[numeric_id]
            else:
                result[comment_id_str] = {"likes": 0, "isLiked": False}
