...
def delete_human_user(db: Session, user_id: int) -> dict:
    """
    删除人类用户及其所有关联数据
    ...
    Returns:
        dict: 删除统计信息，包含删除的帖子数、AI用户数、评论数等
    """
    logger.info(f"开始删除人类用户，ID: {user_id}")
    
    # 获取用户信息
    human_user = get_human_user_by_id(db, user_id)
    if not human_user:
        raise ValueError(f"用户ID {user_id} 不存在")
    
    # 统计删除前的数据量
    deleted_posts_count = db.query(models.Post).filter(models.Post.author_id == user_id).count()
    deleted_ai_users_count = db.query(models.AIUser).filter(models.AIUser.human_user_id == user_id).count()
    
    # 统计该用户直接发布的评论数
    deleted_human_comments_count = db.query(models.Comment).filter(
        models.Comment.sender_id == str(user_id),
        models.Comment.sender_type == "human_user"
    ).count()
    
    # 统计该用户的AI用户发布的评论数
    ai_user_ids = [ai.user_id for ai in db.query(models.AIUser).filter(models.AIUser.human_user_id == user_id).all()]
    deleted_ai_comments_count = 0
    if ai_user_ids:
        deleted_ai_comments_count = db.query(models.Comment).filter(
            models.Comment.sender_id.in_(ai_user_ids),
            models.Comment.sender_type == "ai_user"
        ).count()
    
    total_deleted_comments = deleted_human_comments_count + deleted_ai_comments_count
    ...
    try:
        ...
        # 4. 删除AI用户
        db.query(models.AIUser).filter(
            models.AIUser.human_user_id == user_id
        ).delete(synchronize_session=False)
        ...
        logger.info(f"成功删除人类用户 {human_user.username} (ID: {user_id})")
        logger.info(f"删除统计 - 帖子: {deleted_posts_count}, AI用户: {deleted_ai_users_count}, 评论: {total_deleted_comments}")
        
        return {
            "deleted_user_id": user_id,
            "deleted_username": human_user.username,
